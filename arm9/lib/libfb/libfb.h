...
uint8 getWrapPoints(int x, int y, const void *toDisp, int c_x, int c_y, int c_bx, int c_by, int **pts, uint16 **FP);
...
void dispChar(int x, int y, u32 chr, uint16 *buffer, uint8 max_X, uint8 max_Y);
...
/* This is a generic function for displaying any libfb sprite to any valid 256 by x sized
   uint16 buffer. */
void dispCustomSprite(int x, int y, uint16* sprite, uint32 tc, uint32 colormask, uint16 *bgFlag, uint8 max_X, uint8 max_Y);
